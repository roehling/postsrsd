 ...
 */
int
milter_forward(srs_t *srs, const char *address, char *value, int valuelen,
     ...
     SMFICTX *ctx;
{
	...
	if (milter_forward(srs, priv->mlfi_mailfrom, newaddr, sizeof(newaddr), domain, excludes)) {
	  /* Modify envelope sender for SRS forward processing */
	  /* TODO: Do we need to copy forward ESMTP arguments or are they preserved? */
	  if (smfi_chgfrom(ctx, newaddr, NULL) != MI_SUCCESS) {
            syslog(LOG_MAIL | LOG_INFO, "srs_forward: Failed to change envelope from <%s>",
		   priv->mlfi_mailfrom);
	    return SMFIS_TEMPFAIL;
	  }
	}    
...
int
  milter_main(char *oconn, srs_t *srs_in,
	      char *domain_in, const char **excludes_in)
{
