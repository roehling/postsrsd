...
static void generate_random_address(char *buf, size_t len1, size_t len2)
{
    ...
    while (l1 > 0)
    {
        ...
        if (l1 < len1 && l1 > 1 && buf[i - 1] != '.' && (random() % 16 == 0))
            buf[i - 1] = '.';
        --l1;
    }
    buf[i++] = '@';
    ...
    {
        buf[i++] = 'a' + random() % 26;
        if (l2 < len2 && l2 > 1 && buf[i - 1] != '.' && (random() % 16 == 0))
            buf[i - 1] = '.';
        --l2;
    }
    buf[i++] = 0;
...
#define ASSERT_SRS_OK(...)          \
    if (!run_srs(srs, __VA_ARGS__)) \
        exit(EXIT_FAILURE);
...
int main(int argc, char **argv)
{
    ...
    run_tests_with_secret(
        "string that has 119 bytes, which is excessively long from an "
        "operational point of view but should still work regardless");
    return EXIT_SUCCESS;
}
